...
class PatchMatch:
    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            ix = (torch.arange(W, device="cuda", dtype=torch.float32) - viewpoint_cam.image_width/2) / viewpoint_cam.focal_x
            iy = (torch.arange(H, device="cuda", dtype=torch.float32) - viewpoint_cam.image_height/2) / viewpoint_cam.focal_y
            view_to_nearest_T = (
        ...
        pts_projections = torch.addcmul(
            pts_projections.new_tensor([viewpoint_cam.image_width/2, viewpoint_cam.image_height/2]),
            pts_projections.new_tensor([viewpoint_cam.focal_x, viewpoint_cam.focal_y]),
            pts_projections,
        )
        ...
        rgb_to_gray = torch.tensor([0.299, 0.587, 0.114], dtype=torch.float32, device="cuda")[:,None,None]
        gray_image0 = (viewpoint_cam.original_image.cuda() * rgb_to_gray).sum(0,keepdim=True)
        gray_image1 = (nearest_cam.original_image.cuda() * rgb_to_gray).sum(0,keepdim=True)
        cc, valid_mask = warp_patch_ncc.warp_patch_ncc(
            ...
            viewpoint_cam.image_width/2,
            viewpoint_cam.image_height/2,
            nearest_cam.focal_x,
            nearest_cam.focal_y,
            nearest_cam.image_width/2,
            nearest_cam.image_height/2,
            False,
        )
...
class PatchMatchFast:
    ...
    def __call__(self, gaussians: GaussianModel, render_pkg: dict, viewpoint_cam: Camera, nearest_cam: Camera, iteration=0, depth_normal=None):
        ...
        def sample_depth_func(x,y,z,k,l): assert False
