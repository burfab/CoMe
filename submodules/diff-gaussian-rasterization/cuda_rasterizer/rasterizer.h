...
namespace CudaRasterizer
{
	...
	inline std::string toString(DebugVisualization m) {
		switch(m)
		{
			...
			case DebugVisualization::Depth_Index:
				return "Depth Index";
			case DebugVisualization::Confidence:
				return "Confidence";
			default:
				return "Disabled";
		}
	...
	public:
        public:
          static void extractAccumAlpha(const char *img_buffer, int width, int height,
                                        float *result, bool debug);

          static void markVisible(int P, float *means3D, float *viewmatrix,
                                  float *projmatrix, bool *present);

          static void Compute3DFilter(int P, // num_points
                                      int C, // num_cameras
                                      const float *means3D,
                                      const float *viewmatrices, const int W,
                                      const int H, const float focal_x,
                                      const float focal_y, float *filter_3D);
          ...
          forward(std::function<char *(size_t)> geometryBuffer,
                  ...
                  const float *extra_features, const float *means3D,
                  ...
                  const float *confidences, const float *cov3D_precomp,
                  const float *view2gaussian_precomp, const float *viewmatrix,
                  const float *projmatrix, const float *inv_viewprojmatrix,
                  const float *filter_3d, const float *cam_pos,
                  const float tan_fovx, const float tan_fovy,
                  const bool prefiltered, float *out_color, float *gt_color,
                  int *radii = nullptr, float *max_weights = nullptr,
                  float *cov2Ds = nullptr, bool debug = false);
          ...
          backward(const int P, int D, int M, int R, const float *background,
                   ...
                   const float *view2gaussian_precomp, const float *scales,
                   const float scale_modifier, const float *rotations,
                   const float *confidences, const float *cov3D_precomp,
                   const float *viewmatrix, const float *projmatrix,
                   const float *inv_viewprojmatrix, const float *filter_3d,
          ...
          static int
          integrate(std::function<char *(size_t)> geometryBuffer,
                    std::function<char *(size_t)> binningBuffer,
