...
__device__ void __forceinline__ outputDebugVis(DebugVisualization debugType, float* out_color, int pix_id, uint32_t contributor, float T, float depth, float opacity, float distortion, float extent_loss, float confidence, int range, uint32_t max_contributor, int H, int W)
{
	if (debugType == DebugVisualization::GaussianCountPerPixel)
	{
	...
	{
		out_color[pix_id] = depth;
	}
	else if (debugType == DebugVisualization::Opacity)
	{
		out_color[pix_id] = opacity;
	}
	else if (debugType == DebugVisualization::Distortion)
	{
		out_color[pix_id] = distortion;
	}
	else if (debugType == DebugVisualization::EXTENT_LOSS)
	{
		out_color[pix_id] = extent_loss;
	}
	else if (debugType == DebugVisualization::Depth_Index)
	{
		out_color[pix_id] = max_contributor;
	}
	else if (debugType == DebugVisualization::Confidence)
	{
		out_color[pix_id] = confidence;
		// store the confidence in the confidence channel nonetheless
		out_color[pix_id + CONFIDENCE_OFFSET * H * W] = confidence;
	}
}
