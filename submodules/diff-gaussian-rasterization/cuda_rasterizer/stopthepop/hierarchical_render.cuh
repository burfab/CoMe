    ...
    FF &&fin_function)
{
    ...
    const float2 ray  = { (pixf.x - W / 2.0f) / focal_x, (pixf.y - H / 2.0f) / focal_y };
    ...
    auto fn_loop =[&](auto FIRST_ITER_AS_TYPE, int iter){
        ...
        for (int progress = range.x; progress < range.y; progress += WARP_SIZE)
        {
            ...
            // --- LOCALIZED BATCH PROCESSING BLOCK ---
            // Scoping these variables tightly hints to the compiler that their registers 
            // can be completely reclaimed after this scope block terminates.
            {
                auto global_idx = progress + warp.thread_rank();
                int load_id = -1;
                float con_o_w = 0.0f;
                float V2G[VIEW2GAUSSIAN_OFFSET];

                if(global_idx < range.y && is_bit_set_outter) {
                    load_id = point_list[global_idx];
                }
                if(load_id != -1){
                    con_o_w = conic_opacity[load_id].w;
                    // Inlining the load loop cuts the lambda overhead
                    #pragma unroll
                    for(int i = 0; i < VIEW2GAUSSIAN_OFFSET; i++) {
                        V2G[i] = view2gaussian[load_id * VIEW2GAUSSIAN_OFFSET + i];
                    }
                }
                ...
                while (mask_to_process != 0) {
                    ...
                    const float target_con_o_w = __shfl_sync(0xFFFFFFFF, con_o_w, j, WARP_SIZE);
                    ...
                    if (is_bit_set && active) {
                        ...
                        const float alpha = min(0.99f, target_con_o_w * G);
            ...
            } // Register lifetimes for load_id, con_o_w, and V2G explicitly signal end-of-life here
        }

        // --- TAIL RECOVERY BLOCK ---
        // Postponing the load of global variables to exactly where they are consumed 
        // stops the compiler from holding them during the long loop sequence.
        if (pixpos.x < W && pixpos.y < H)
        {
            // const glm::mat4 inverse_vp = loadMatrix4x4(projmatrix_inv); // Completely unreferenced! Deleted.
            const float3 o = *cam_pos; 
...
#pragma unroll
          ...
          if (warp.any(active)) {
            ...
            if (warp.any(active)) {
              ...
              // drain front
              while (active && (fill_counters & FillHeadMask) != 0) {
                blend_one();
              }
            }
          }
...
template <int32_t CHANNELS, int HEAD_WINDOW, int MID_WINDOW,
          bool CULL_ALPHA = true, bool EXACT_DEPTH = false,
          bool ENABLE_DEBUG_VIZ = false, bool CONSIDER_MAX_WEIGHT = false,
          bool GSGS_TWO_PASS = false>
__global__ void __launch_bounds__(16 * 16, 4)
    sortGaussiansRayHierarchicalCUDA_renderForward(
        ...
        float *__restrict__ max_weights, float *__restrict__ cov2Ds,
        CudaRasterizer::DebugVisualization debugType,
        float *__restrict__ out_color, float *__restrict__ gt_color) {
  ...
  auto store_function = [](const uint2 &, int coll_id, float G, float alpha,
                           float depth) { return alpha; };
  auto blend_function = [&](auto FIRST_ITER_AS_TYPE, const uint2 &pixpos,
                            BlendData &blend_data, int id, float alpha, float t,
                            const float *view2gaussian_j, float2 ray,
                            CudaRasterizer::DebugVisualization debugType,
    ...
    for (int ch = 0; ch < CHANNELS; ch++) {
      ...
      // +++++++++Variance Loss ++++++++++
      blend_data.variance += weight * (blend_data.gt_color[ch] - rgb[ch]) *
    ...
    const float normal_normalized[3] = {
    ...
    if (blend_data.T > 0.5f) {
      ...
      if (t > blend_data.depth) {
        float min_value = (AA * blend_data.depth * blend_data.depth +
    ...
    if constexpr (GSGS_TWO_PASS) {
      ...
      // https://arxiv.org/pdf/2403.17888.pdf
      const float max_t = t;
      const float mapped_max_t = (far_plane * max_t - far_plane * NEAR_PLANE) /
                                 ((far_plane - NEAR_PLANE) * max_t);

      float A = 1 - blend_data.T;
      float error = mapped_max_t * mapped_max_t * A + blend_data.dist2 -
  ...
  auto fin_function = [&](const uint2 &pixpos, BlendData &blend_data,
    ...
    for (int ch = 0; ch < CHANNELS; ch++) {
      blend_data.variance += blend_data.T *
                             (blend_data.gt_color[ch] - bg_color[ch]) *
                             (blend_data.gt_color[ch] - bg_color[ch]);
    }
    // +++++++++Variance Loss ++++++++++
    ...
    if constexpr (!ENABLE_DEBUG_VIZ) {
      ...
      out_color[OCCUPATION_OFFSET * H * W + pix_id] =
          ...
          max((1 - blend_data.T),
              1e-3f); // / fmaxf((float)blend_data.blend_contributor, 1.f);
                      /*
                    out_color[OCCUPATION2_OFFSET * H * W + pix_id] =
                            blend_data.occupation2 / max((1 - blend_data.T), 1e-3f);// /
                    fmaxf((float)blend_data.blend_contributor, 1.f);
                      */
    } else {
      outputDebugVis(debugType, out_color, pix_id, blend_data.contributor,
                     blend_data.T, blend_data.depth, blend_data.opacity,
                     blend_data.distortion, blend_data.extent_loss,
                     blend_data.confidence, range, blend_data.max_contributor,
                     H, W);
    }
#if (DEBUG_HIERARCHICAL & 0x200) != 0
    if (pixpos.x == debug_target_pixel.x && pixpos.y == debug_target_pixel.y) {
    ...
    if (pixpos.x < 10 && pixpos.y < 10) {
      ...
      printf("[%d, %d]: depth: %.3f, depth point %.3f %.3f %.3f\n", pixpos.y,
             pixpos.x, blend_data.C[CHANNELS * 2], depth.x, depth.y, depth.z);
    }
#endif
    return false;
  };
...
*/
				...
				if(!blend_data.in_range || 
					++blend_data.contributor >= blend_data.forward_max_blend_contributor) return false;
                return true;
              };
          auto fin_function = [&](const uint2 &pixpos, BlendData &blend_data,
                                  CudaRasterizer::DebugVisualization debugType,
	...
	float* __restrict__ gt_color)
{
          ...
          struct BlendData {
			...
			float &L_distortion_last_iter(){return interval;}
          };
          ...
          auto blend_function =
              [&](auto FIRST_ITER_AS_TYPE ,const uint2 &pixpos, BlendData &blend_data, int id,
				...
				if(++blend_data.contributor >= blend_data.forward_max_blend_contributor) return false;
				else return true;
              };
          auto fin_function = [&](const uint2 &pixpos, BlendData &blend_data,
                                  CudaRasterizer::DebugVisualization debugType,
...
#pragma unroll
          ...
          GaussiansRayEvaluation<BINARY_SEARCH_ITERATIONS, HEAD_WINDOW, MID_WINDOW,
              ...
              points_xy_image, cov3Ds_inv, projmatrix_inv, cam_pos,
              conic_opacity, CudaRasterizer::DebugVisualization::Disabled, prep_function, 
              blend_function, 
			  fin_function);
}
...
__global__ void __launch_bounds__(16 * 16) sortGaussiansRayHierarchicalCUDA_binarySearchBackward(
	const uint2* __restrict__ ranges,
	const uint32_t* __restrict__ point_list, 
	const uint64_t * __restrict__ per_pixel_bit_mask,
	int W, int H,
	float focal_x, float focal_y, 
	const float far_plane,
	...
	float* __restrict__ first_pass_bw)
{
	...
	struct BlendData{
		int contributor;
		int max_blend_contributor_forward;
		float depth;
		float dT_dtmedian;
		float T;
	};
	...
	auto prep_function = [&](bool inside, const uint2& pixpos, const float3 raydir)
		{
			uint32_t pix_id = W * pixpos.y + pixpos.x;
			BlendData bd;
			bd.max_blend_contributor_forward = inside ? (n_contrib[pix_id] >> 16) & 0xFFFF : 0;
			bd.contributor = 0;
			bd.depth = inside ? pixel_colors[DEPTH_OFFSET * H * W + pix_id] : 0;
			bd.dT_dtmedian = 0.0f;
			bd.T = 1.0f;
	...
	auto blend_function = [&](auto FIRST_ITER_AS_TYPE,const uint2& pixpos, BlendData& blend_data, int global_id, float G, float alpha,float t, const float* view2gaussian_j, float2 ray, CudaRasterizer::DebugVisualization debugType, float3 normal_, float4 ABC_)
		{
            constexpr bool FIRST_ITER = decltype(FIRST_ITER_AS_TYPE)::value;
			[[maybe_unused]] const float AA = ABC_.x;
			[[maybe_unused]] const float BB = ABC_.y;
			...
			float dT_dtmed;
			const float T_tmed = transmittance_helper.dT_dt(blend_data.depth, dT_dtmed);
			blend_data.dT_dtmedian += 1.f/(T_tmed) * dT_dtmed;
			blend_data.T *= T_tmed;
			if(++blend_data.contributor >= blend_data.max_blend_contributor_forward) return false;
			return true;
		};
	...
	auto fin_function = [&](const uint2& pixpos, BlendData& blend_data, CudaRasterizer::DebugVisualization debugType, int range, float3 o)
		{
			uint32_t pix_id = pixpos.y * W + pixpos.x;
			blend_data.dT_dtmedian = 1.f / fmaxf(-0.5f*(blend_data.dT_dtmedian), 1e-7f);
			first_pass_bw[H*W*0+pix_id] = blend_data.dT_dtmedian;
			return false;
		};


	GaussiansRayEvaluation<1, HEAD_WINDOW, MID_WINDOW, CULL_ALPHA>(
		ranges, point_list, per_pixel_bit_mask, W, H, focal_x, focal_y, view2gaussian,
		points_xy_image, cov3Ds_inv, projmatrix_inv, cam_pos,
		conic_opacity, CudaRasterizer::DebugVisualization::Disabled, prep_function, 
		blend_function, 
...
template <int32_t CHANNELS, int HEAD_WINDOW, int MID_WINDOW, bool CULL_ALPHA, bool DETACH_ALPHA, bool EXACT_DEPTH, bool GSGS_TWO_PASS, bool RENDER_GEOMETRY>
__global__ void __launch_bounds__(16 * 16, 4) sortGaussiansRayHierarchicalCUDA_backward(
	...
	const uint64_t * __restrict__ per_pixel_bit_mask, int W, int H, float focal_x, float focal_y, 
	const float far_plane, const bool detach_alpha_extent, const bool include_alpha, 
	const float* view2gaussian, const float* __restrict__ bg_color, const float2* __restrict__ points_xy_image, 
	...
	const uint32_t* __restrict__ n_contrib, const float* __restrict__ pixel_colors, const float* __restrict__ gt_colors, 
	const float* __restrict__ dL_dpixels, const float* __restrict__ first_pass_bw, float3* __restrict__ dL_dmean2D, 
	float4* __restrict__ dL_dconic2D, float* __restrict__ dL_dopacity, float* __restrict__ dL_dcolors, 
	float* __restrict__ dL_dconfidences, float* dL_dview2gaussian)
{
			const float ddelx_dx = 0.5 * W;
			const float ddely_dy = 0.5 * H;
			// int num_blends = 0;
			...
			struct BlendData
			{
				...
				auto prep_function = [&](bool inside, const uint2& pixpos, const float3 raydir)
				{
					...
					if(inside)
					{
						...
						if constexpr (GSGS_TWO_PASS && RENDER_GEOMETRY){
							bd.dL_dmt_dT_dtm = bd.dL_dmax_depth * first_pass_bw[H*W*0+pix_id];
						}else {
							bd.dL_dmt_dT_dtm = 0.f;
						}
					}
			...
			auto fin_function = [&](const uint2& pixpos, BlendData& blend_data, CudaRasterizer::DebugVisualization debugType, int range, float3 o)
				...
				if (blend_data.depth_global_id != (uint32_t)-1 &&
					...
					atomicAdd(&dL_dview2gaussian
								[blend_data.depth_global_id * VIEW2GAUSSIAN_OFFSET + 5],
							dL_dnormal[2]);
					atomicAdd(&dL_dview2gaussian
								[blend_data.depth_global_id * VIEW2GAUSSIAN_OFFSET  + 6],
							dL_dB * 2.f * blend_data.ray.x);
					atomicAdd(&dL_dview2gaussian
								[blend_data.depth_global_id * VIEW2GAUSSIAN_OFFSET  + 7],
							dL_dB * 2.f * blend_data.ray.y);
					atomicAdd(&dL_dview2gaussian
								[blend_data.depth_global_id * VIEW2GAUSSIAN_OFFSET  + 8],
							dL_dB * 2.f);
					atomicAdd(&dL_dview2gaussian
								[blend_data.depth_global_id * VIEW2GAUSSIAN_OFFSET  + 9],
							dL_dC); // NEW
