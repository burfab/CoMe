...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], skip_train=False, skip_test=False, MCMC_init=False, add_bbox_faces=False):
        ...
        if os.path.exists(os.path.join(args.source_path, "cameras.json")):
            scene_info = sceneLoadTypeCallbacks["iPhone"](args.source_path, args.images, args.eval, init_type=args.init_type)
    ...
    def save(self, iteration, appearance_embedding, segmentation_network, deform_model):
        ...
        try:
            ...
            with open(os.path.join(point_cloud_path, "deform_model.pth"), 'wb') as file:
                torch.save(deform_model, file)
