...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", array_name="frames"):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            if not os.path.exists(image_path):
                image_path = os.path.join(path, "..", "resized_undistorted_images", frame["file_path"] + extension)
...
def readScanNetSceneInfo(path, eval, init_type="sfm", num_pts=100000):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(os.path.join(path, "dslr", "nerfstudio"), "transforms_undistorted.json", True, "")
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(os.path.join(path, "dslr", "nerfstudio"), "transforms_undistorted.json", True, "", array_name="test_frames")
    ...
    if init_type == "sfm":
        ...
        bin_path = os.path.join(path, "dslr/colmap/points3D.bin")
        txt_path = os.path.join(path, "dslr/colmap/points3D.txt")
        if not os.path.exists(ply_path):
