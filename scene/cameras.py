...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
                 image_name, uid, resolution,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, data_device = "cuda", seg_mask=None):
        ...
        if seg_mask is not None:
            self.seg_mask = seg_mask.to(self.data_device)
        else:
            self.seg_mask = None
...
class CustomCam:
    def __init__(self, width, height, fovy, fovx, extr, znear=0.01, zfar=10):
        ...
        self.full_proj_transform_inverse = (
            self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))
        ).squeeze(0).inverse()
        # TODO: actually make a PR for splatviz
        self.camera_center = self.world_view_transform.inverse()[3, :3]