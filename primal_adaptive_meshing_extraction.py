...
# Scene imports
from arguments import SplattingSettings, ExtendedSettings, GlobalSortOrder
...
# Loading imports
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "eval_module", "TNTUniScanEvals"))
from eval_module.TNTUniScanEvals.uniform_sampling_eval import (
...
def compute_global_occupancy(points_input, views, gaussians, pipeline, kernel_size, integrate_func, mask_bg_threshold=0.01, chunk_size=1_000_000, verbose_views=False, device="cuda"):
    ...
    for point_chunk in torch.chunk(points, points.shape[0] // chunk_size + 1):
        ...
        for view in tqdm(views, desc="Rendering progress") if verbose_views else views:
            ...
            if not WARNED_INSIDE_DISABLED: print("WARNING: inside disabled"); WARNED_INSIDE_DISABLED = True
            if False and "inside" in ret:
                has_inside_key = True
                ...
                final_weight_chunk = torch.where(
                    ...
                    torch.min(ret["transmittance"], final_weight_chunk),
                    torch.where(
                ...
                has_inside_key = False
                final_weight_chunk = torch.min(final_weight_chunk, ret["transmittance"])
...
def create_global_occupancy_func(args: ArgumentParser, return_details=False):
    ...
    from gaussian_renderer import evaluate_transmittance as integrate_func
    ...
    global_occupancy_func = lambda query_points: compute_global_occupancy(
        ...
        pipeline, 
        kernel_size, 
        partial(integrate_func, splat_args = splat_args),
...
def gradient_descent_refinement(points: np.ndarray, occupancy_function: Callable, grad_occupancy_function: Callable, args: ArgumentParser, chunk_size: int = 1_000_000, device: str = "cuda"):
    ...
    for i in tqdm(range(args.n_steps), desc="Gradient Descent Steps"): # we use this as number of gradient descent steps
        ...
        if args.plot_vacancy_histogram:
            directory = os.path.join(get_mesh_path(args)["dir"], "vacancy_opt")
            if not os.path.exists(directory): os.makedirs(directory)
            plot_histogram(occupancy_values.cpu().numpy(), filename=os.path.join(directory, f"histogram_occupancy_values_{i}.png"), title="Histogram of Occupancy Values", iso_value=args.iso_surface_value)
...
def get_mesh_path(args):
    ...
    return {
        "inpath": mesh_path, 
        "dir": mesh_dir,
        "outpath": os.path.join(mesh_dir, "primal_" + mesh_name + "" + mesh_ext)
    }
...
def load_mesh(args: ArgumentParser, scene_pckg: Dict[str, Any]):
    ...
    '''
    # Load mesh
    mesh_path = get_mesh_path(args)["inpath"]
...
def main(args):
    ...
    outpath = get_mesh_path(args)["outpath"]

    if args.save_candidate_points:
        print(f"[INFO] Saving candidate points to {outpath.replace('.ply', '_candidate_points.ply')}")
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(points)
        o3d.io.write_point_cloud(outpath.replace(".ply", "_candidate_points.ply"), pcd)

    # Extract mesh
    surface_verts, surface_faces = mesh_from_points_and_occupancy(points, args, occupancy_func)
    
    # Save mesh
    print(f"[INFO] Saving mesh to {outpath}")
    export_mesh(surface_verts, surface_faces, outpath, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mesh_name", default="mesh_2pivots.ply")
    parser.add_argument("--subfolder_prefix", default="ours")
    
    
    parser.add_argument("--max_points", type=int, default=1_000_000, help="Max points to process")
